...
export class CustomFilter extends Component{
    ...
    setObjFromCookies(){
        let customDomainFacets_from_cky = getObjectFromSession('CFilter' + this.props.options.ks_dashboard_id)
    ...
    clearFacets_cookies(ev){
        eraseSessionItem('CFilter' + this.props.options.ks_dashboard_id)
        this.customDomainFacets = {}
    }
    ...
    async applyFilters(ev) {
        ...
        if(!domainsToUpdate) return;
        this.clearRows();
        setObjectInSession('CFilter' + this.props.options.ks_dashboard_id,
    ...
    async validate_and_ReturnDomainsToUpdate(models_domain){
        ...
        for(const [model, model_domain] of Object.entries(models_domain)){
            ...
            if (isValid === undefined) {
                isValid = await this.rpc("/web/domain/validate", { model: model, domain, });
            }
            if (!isValid) {
