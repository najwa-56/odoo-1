...
class gen_sale(models.TransientModel):
	...
	def make_sale(self, values):
		...
		if sale_search:
			...
			if not sale_id.is_import:
				sale_id.is_import = True
	...
	def make_order_line(self, values, sale_id):
		...
		if self.import_prod_option == 'barcode':
			...
			if prefix:
				...
				if res == True:
					product_search = product_obj.search([('barcode','=',str(values['product']).rstrip('0').rstrip('.') if '.' in str(values['product']) else str(values['product']))])
				else:
					product_search = product_obj.search([('barcode', '=',values['product'])])
	...
	def import_sale(self):
		...
		if self.import_option == 'csv':
			try:
				keys = ['order', 'customer', 'pricelist','product', 'quantity', 'uom', 'description', 'price','user','tax','date','discount']
