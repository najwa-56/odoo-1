# Report to know the FIFO Valuation of Products.
# FIFO is an inventory valuation method where the first items added to the inventory are assumed to be the first ones sold
...
class FIFOValuationReport(models.TransientModel):
    _name = 'fifo.valuation'
    _description = 'FIFO Valuation Report'
    ...
    def name_get(self):
        res = []
        for record in self:
            name = _('FIFO Valuation Report')
            res.append((record.id, name))
        return res

    def generate_xls_report(self):
        products = self.env['product.product'].search([])
        sorted_products = sorted(products, key=lambda p: p.create_date)

        workbook = xlwt.Workbook()
        worksheet = workbook.add_sheet('FIFO Valuation')
        header_style = xlwt.easyxf('font: bold on; align: horiz center;')
        cell_style = xlwt.easyxf('align: horiz center;')

        worksheet.write_merge(0, 0, 0, 8, 'FIFO Valuation Report', header_style)
        ...
        filename = 'FIFO Valuation Report.xls'
    ...
    def generate_pdf_report(self):
        return self.env.ref('inventory_reports_adv_axis.fifo_valuation_pdf_report').report_action(self, config=False)
    ...
    @api.model
    def fifo_data_js(self,activeid):
