...
class HREndServiceBenifits(models.Model):
    ...
    def _compute_period(self):
        for record in self:
            if record.start_date:
                ...
                period_days = relativedelta(record.date, start_date)
        ...
        for record in self:
            ...
            if contract_id:
                ...
                if record.payment_type == 'wage_allowance':
                    allowances = contract_id.travel_allowance + contract_id.meal_allowance + + contract_id.medical_allowance  + contract_id.other_allowance
    ...
    start_date = fields.Date(string="Hiring Date", related='employee_id.start_date', store=True)
    ...
    total_holiday_deserved_amount = fields.Float(string="Total Holidays Deserved Amount",
    ...
    total_deserved_amount = fields.Float(string="Total Deserved Amount", compute='_compute_total_deserved_amount',
                                         store=True)
    total_taken_amount = fields.Float(string="Previously Disbursed Amount", compute='_compute_total_taken_amount',
                                      store=True)
    ...
    def action_validate(self):
        for record in self:
            ...
            template = False
            if recipient_partners and mail_server:
                template = self.env['ir.model.data']._xmlid_to_res_id('hr_end_service_benefits.email_es_request_payment_request')
            if template:
                mail_template = self.env['mail.template'].browse(template)
                mail_id = mail_template.send_mail(record.id)
                mail = self.env['mail.mail'].browse([mail_id])
