...
class Settlement(models.TransientModel):
    ...
    @api.depends('payment_type', 'settlement_journal_id',)
    def _compute_payment_method_line_fields(self):
        for pay in self:
            pay.available_payment_method_line_ids = pay.settlement_journal_id._get_available_payment_method_lines(pay.payment_type)
    ...
    def settle_employee_reward(self):
        for record in self:
            ...
            if not record.employee_id.work_contact_id.property_account_payable_id:
                raise exceptions.ValidationError(
            ...
            credit_account_id = record.employee_id.work_contact_id.property_account_payable_id.id
            if debit_account_id and record.employee_id.work_contact_id:
                debit_line = (0, 0, {
