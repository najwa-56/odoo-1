...
class PurchaseOrderLine(models.Model):
    ...
    class PurchaseOrderLine(models.Model):
        ...
        purchase_multi_uom_id = fields.Many2one("product.multi.uom.price", string="Custom UOM",
                                                domain="[('id', 'in', selected_uom_ids)]")
        purchase_multi_uom_cost = fields.Float(string="UOM Cost", related='purchase_multi_uom_id.cost')
        price_unit = fields.Float(string='Unit Price', compute='_compute_price_unit_and_date_planned_and_name',
                                  store=True)
        ...
        def _compute_price_unit_and_date_planned_and_name(self):
            for line in self:
                ...
                if line.purchase_multi_uom_cost:
                    # Compute price_unit with tax included from purchase_multi_uom_cost
                    price_without_tax = line.purchase_multi_uom_cost
                ...
                if not line.product_id.seller_ids.filtered(
                        lambda s: s.partner_id == line.order_id.partner_id) and line.price_unit and line.product_uom == line._origin.product_uom:
                    continue
                ...
                if line.product_id.seller_ids:
                    ...
                    # Set name if not default or not set
                    default_names = []
