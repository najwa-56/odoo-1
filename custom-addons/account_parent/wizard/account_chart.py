...
class OpenAccountChart(models.TransientModel):
	...
	def _amount_to_str(self, value, currency):
		return self.format_amount_to_str(value, currency)

	def format_amount_to_str(self, value, currency):
		""" workaround to apply the float rounding logic of t-esc on data
		prepared server side """
		fmt = "%.{0}f".format(currency.decimal_places)
		lang = get_lang(self.env)
		formatted_amount = lang.format(
			fmt, currency.round(value), grouping=True, monetary=True).replace(
		...
		return Markup('{pre}{0}{post}').format(
	...
	def float_html_formatting(self, value, company):
		...
		if (
				'output_format' in self._context.keys() and
				self._context.get('output_format') == 'xls'):
	...
	def line_data(self, level, wiz_id, parent_id, account):
		return {
			...
			'debit': self.float_html_formatting(account.debit, account.company_id),
			'credit': self.float_html_formatting(account.credit, account.company_id),
			'balance': self.float_html_formatting(account.balance, account.company_id),
			...
			'initial_balance': self.float_html_formatting(
				account.initial_balance, account.company_id),
			'ending_balance': self.float_html_formatting(
				account.initial_balance + account.balance, account.company_id),
			'db': account.debit,
			'cr': account.credit,
			'bal': account.balance,
	...
	def at_line_data(
		...
		data.update({
			...
			'company_obj': company,
			'debit': '%s' % self.float_html_formatting(total_debit, company),
	...
	def _at_lines(self, wiz_id, line_id, level=1):
		...
		at_datas = list(filter(
			lambda x: x['parent_id'] == line_id, self.account_type_data()))
			...
			if display_account == 'movement':
				if sum(accounts.mapped('credit')) or sum(accounts.mapped('debit')):
					final_vals += [self.at_line_data(
						at_data, level, wiz_id=wiz_id, parent_id=line_id,
						accounts=accounts)]
			else:
				final_vals += [self.at_line_data(
					at_data, level, wiz_id=wiz_id, parent_id=line_id,
	...
	def get_lines(self, wiz_id=None, line_id=None, **kw):
		...
		res = getattr(report_obj, 'get_%s_lines' % report_type)(wiz_id, line_id, level)
		...
		for line in self.get_lines(self.id, line_id=line_id, level=level):
			...
			if line['type'] == 'view':
				result.extend(self.get_all_lines(
					line_id=line['model_id'], level=line['level']+1))
		return result
	...
	def get_pdf(self, wiz_id):
		...
		return self.env['ir.actions.report']._run_wkhtmltopdf(
			...
			specific_paperformat_args={
				'data-report-margin-top': 17, 'data-report-header-spacing': 12}
		)
	...
	def show_journal_items(self, wiz_id=None, line_id=None, acc_name=''):
		...
		report_type = context.get('report_type', 'account_type')
		account_ids = list()
		if report_type == 'account_type':
			at_datas = list(
				filter(lambda x: x['parent_id'] == line_id, report_obj.account_type_data()))
			for at_data in at_datas:
				account_ids += report_obj.get_at_accounts(at_data, context).ids
			if not at_datas:
				at_datas = list(
					filter(lambda x: x['id'] == line_id, report_obj.account_type_data()))
				for at_data in at_datas:
					account_ids += report_obj.get_at_accounts(at_data, context).ids
		else:
			if line_id:
				account_ids.append(line_id)
			account_ids += report_obj.get_accounts(line_id, context).ids
