...
class AccountAccount(models.Model):
	...
	def _compute_account_root(self):
		...
		# This field should have been a char, but the aim is to use
		# it in a side panel view with hierarchy,
		...
		for record in self:
			if record.account_type != 'view':
				record.root_id = record.code and (
						ord(record.code[0]) * 1000 + ord(record.code[1:2] or '\x00')
				) or False
			else:
...
class AccountJournal(models.Model):
	...
	def _prepare_liquidity_account_vals(self, company, code, vals):
		res = super()._prepare_liquidity_account_vals(
			company=company, code=code, vals=vals)
		account_code_prefix = ''
		if res['account_type'] == 'asset_cash':
			account_code_prefix = (
					company.cash_account_code_prefix or
					company.bank_account_code_prefix or '')
		parent_id = self.env['account.account'].with_context(
