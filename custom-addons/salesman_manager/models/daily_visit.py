...
class DailyVisit(models.Model):
    ...
    order_id = fields.Many2one(comodel_name='sale.order', string="رقم الطلب", ondelete='cascade', index=True, copy=False)
    ...
    def create(self, vals_list):
        ...
        return super(DailyVisit, self).create(vals_list)
    ...
    total_consumption_amount = fields.Float(
        ...
        store=False  # Set to True if you want to store the result
    )
    ...
    def _compute_total_consumption_amount (self):
        for visit in self:
            if visit.partner_id:
                # Total consumption from Sale Orders
                sale_orders = self.env['sale.order'].search( [
                    ('partner_id', '=', visit.partner_id.id)
                ] )
                total_consumption = sum( s.total_consumption_amount_perSO for s in sale_orders )

                # Total payments from Customer Payments (account.payment)
                payments = self.env['account.payment'].search( [
                    ('partner_id', '=', visit.partner_id.id),
                    ('state', '=', 'posted'),
                    ('payment_type', '=', 'inbound')  # Only incoming (customer) payments
                ] )
