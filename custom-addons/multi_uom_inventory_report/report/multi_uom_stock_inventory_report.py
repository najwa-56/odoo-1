...
class emulti_uom_inventory_report_stock_inventory_report(models.AbstractModel):
    _name = 'report.multi_uom_inventory_report.multi_uom_stock_report_pdf'
    _description = "Multi Report Stock Inventory"

    @api.model
    def _get_report_values(self, docids, data=None):
        report = self.env['ir.actions.report']._get_report_from_name('multi_uom_inventory_report.multi_uom_stock_inventory_report')
        record_id = data['form']['id'] if data and data['form'] and data['form']['id'] else docids[0]
        records = self.env['multi.uom.wizard.stock.inventory'].browse(record_id)
        return {
    ...
    def get_location_wise_product(self, record, warehouse, location_ids):
        ...
        for product in product_ids:
            ...
            location_header_data_dict = {
                'beg_qty': 0, 'product_qty_in': 0, 'product_qty_out': 0,
                'product_qty_internal': 0, 'product_qty_adjustment': 0,
                'product_ending_qty': 0, 'product_id': product,
                'uom_data': []  # Store UOM data inside the product
            }
            ...
            for location in location_ids:
                ...
                location_data_dict = {
                    ...
                    'product_qty_internal': 0, 'product_qty_adjustment': 0,
                    'product_ending_qty': beg_qty, 'location_id': location,
                    'uom_data': []  # Store UOM data inside each location-wise entry
                }
