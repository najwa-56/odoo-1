...
class multi_uom_wizard_stock_inventory(models.TransientModel):
    _name = 'multi.uom.wizard.stock.inventory' 
    ...
    def print_report(self):
        ...
        return self.env.ref('multi_uom_inventory_report.multi_uom_action_stock_inventory_template').report_action(self, data=datas)
    ...
    def print_xls_report(self):
        ...
        report_stock_inv_obj = self.env['report.multi_uom_inventory_report.multi_uom_stock_report_pdf']
                ...
                if not self.group_by_categ:
                    ...
                    for key,product_val in products_data.items():
                        ...
                        for uom_row in product_val.get('uom_data', []):  # Ensure UOM data exists
                            ...
                            worksheet.write(rows, 5, abs(uom_row['product_qty_internal']), header_data_format)
                            worksheet.write(rows, 6, uom_row['product_qty_adjustment'], header_data_format)
                            worksheet.write(rows, 7, uom_row['product_ending_qty'], header_data_format)
                            rows += 1
